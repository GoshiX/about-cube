...
def generate_handdrawn_cube():
    fig = plt.figure(figsize=(10, 10), dpi=dpi)
    ax = fig.add_subplot(111, projection='3d')

    # Define cube vertices with small random deviations to make it look hand-drawn
    ...
    vertices = base_vertices + np.random.normal(0, 0.1, base_vertices.shape)
    ...
    # Plot cube with slight wobbliness
    for edge in edges:
        # Add intermediate points with small deviations
        start = edge[0]
        end = edge[1]
        
        # Create 3-5 points along the line with small deviations
        ...
        for i in range(num_points + 1):
            ...
            if 0 < t < 1:  # Don't add noise to endpoints
                point += np.random.normal(0, 0.03, 3)
            points.append(point)
        
        # Convert to array and plot with varying line width
    ...
    # final_path = "handdrawn-photo/" + ";".join(f"{coord[0]},{coord[1]}" for coord in pixel_coords) + ".png"
    final_path = "test.png"
    image.save(final_path)

    plt.close(fig)

    print(f"Изображение с кубом от руки сохранено как '{final_path}'")
    return pixel_coords, final_path

# for i in range(1000):
generate_handdrawn_cube()
    # print(i+1)